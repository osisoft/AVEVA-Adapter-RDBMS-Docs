...
Version  [!include[product-version](../main/shared-content/_includes/inline/product-version.md)] represents the initial release for PI Adapter for RDBMS. This product collects time-series data and sends it to configured OMF (OSIsoft Message Format) endpoints in OSIsoft Cloud Services or PI Servers. The time-series data can originate from any RDBMS that supports Open Database Connectivity (ODBC) drivers.
