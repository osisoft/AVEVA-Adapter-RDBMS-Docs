...
| **RequestInterval** | Optional | `string` | Specifies the query interval for history recovery. This will be used to replace the ?ST? and ?ET? parameters in your query. For example, if the **RequestInterval** is set to `1h`, the difference between ?ST? and ?ET? in each query will be 1 hour. For more information, see also [Automatic history recovery](xref:AutomaticHistoryRecovery) and [Queries configuration](xref:PIAdapterForRDBMSQueriesConfiguration). | 
