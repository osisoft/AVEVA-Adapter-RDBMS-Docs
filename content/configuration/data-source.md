...
| **RequestInterval** | Optional | Specifies the interval for an automatic history recovery. For example, if an outage lasts six hours and the **RequestInterval** is set to `1h`, recovered data will be grouped into six recovery items. For more information, see also [Automatic history recovery](xref:AutomaticHistoryRecovery). | 
