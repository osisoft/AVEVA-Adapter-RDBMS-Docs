...
| **UserName** | Optional | `string` | Optional username to be used in ConnectString. This value will replace the [username] token.<br><br>**Note**: If you are using Windows authentication, you must add your domain to the UserName<br><br>Example: YourDomain\\YourUserName.<br><br>**This user should be configured to have the minimum permissions needed to read the desired data from your database. It is not recommended to use an admin user or even a user with write permissions.** |
