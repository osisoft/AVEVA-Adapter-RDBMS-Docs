...
| **ConnectTimeout** | Optional | `string` | Optional timeout for connections to the data source.<br><br>Expected format: `HH:MM:SS.###`* |
...
| **RequestInterval** | Optional | `string` | Maximum period of time for which the adapter will request data at once during history recovery. It is advised to set this property when doing history recovery so the adapter and data source do not get overloaded.<br><br>Expected format is `HH:MM:SS.###`* |
